    ...
    const fetchReviews = async () => {
      ...
      try {
        const [statsRes, reviewsRes] = await Promise.all([
          fetch(`http://localhost:5000/api/reviews/product/${id}/stats`),
        ...
        if (user?.id) {
          const userRes = await fetch(`http://localhost:5000/api/reviews/product/${id}/user/${user.id}`);
          if (userRes.ok) {
                    ...
                    <div key={r._id} className="rounded-xl p-4 glass-strong">
                      ...
                      <div className="mt-2 flex items-center gap-2 text-xs text-muted-foreground">
                        <button
                          ...
                          onClick={async () => {
                            const res = await fetch(`http://localhost:5000/api/reviews/${r._id}/helpful`, { method: 'POST' });
                            if (res.ok) {
                        ...
                        {user?.id === r.userId && (
                          <button
                            ...
                            onClick={async () => {
                              const res = await fetch(`http://localhost:5000/api/reviews/${r._id}`, {
                                method: 'DELETE',
                                headers: { 'Content-Type': 'application/json' },
                              ...
                              if (res.ok) {
                                ...
                                const statsRes = await fetch(`http://localhost:5000/api/reviews/product/${id}/stats`);
                                if (statsRes.ok) setReviewStats(await statsRes.json());
                    ...
                    onClick={async () => {
                      ...
                      try {
                        const response = await fetch(
                          `http://localhost:5000/api/wishlist/${user.id}/${endpoint}/${id}`,
                          { method }
                        );
